from ._anvil_designer import CALIDAD_CONTROLDOCUMENTOS_VISOR_GOOGLE_APP_copyTemplate
from anvil import *
...
class CALIDAD_CONTROLDOCUMENTOS_VISOR_GOOGLE_APP_copy(CALIDAD_CONTROLDOCUMENTOS_VISOR_GOOGLE_APP_copyTemplate):
  datos = {}
  ...
  def drop_down_tipo_entrada_change(self, **event_args):
    self.text_area_filas.text = ""
    self.text_area_columnas.text = ""
    self.text_area_filas.placeholder = ""
    self.text_area_columnas.placeholder = ""
    self.text_area_filas.visible = False
    self.text_area_columnas.visible = False
    if self.drop_down_tipo_entrada.selected_value not in (None, "RESPUESTA CORTA","PÁRRAFO","SELECTOR FECHA","SELECTOR FECHA Y HORA","SUBIR ARCHIVO"):
      self.text_area_filas.visible = True
      if self.drop_down_tipo_entrada.selected_value in ("CASILLA(S) DE VERIFICACIÓN","OPCIÓN MÚLTIPLE","LISTA DESPLEGABLE"):
        self.text_area_filas.placeholder = "Opción 1\nOpción 2\nOpción 'N'..."
      else:
        self.text_area_filas.placeholder = "Fila 1\nFila 2\n..."
        self.text_area_columnas.visible = True
        self.text_area_columnas.placeholder = "Columna 1\nColumna 2\nOpción 'N'..."

  def button_agregar_click(self, **event_args):
    error = ""
    if self.text_area_pregunta.text == "":
      error += "\n•Pregunta."
    if self.drop_down_tipo_entrada.selected_value == None:
      error += "\n•Tipo de Entrada."
    elif self.drop_down_tipo_entrada.selected_value not in (None, "RESPUESTA CORTA","PÁRRAFO","SELECTOR FECHA","SELECTOR FECHA Y HORA","SUBIR ARCHIVO"):
      if len(self.text_area_filas.text) == 0:
       if self.drop_down_tipo_entrada.selected_value in ("CASILLA(S) DE VERIFICACIÓN", "OPCIÓN MÚLTIPLE", "LISTA DESPLEGABLE"):
         error += "\n•OPCIONES DE ENTRADA"
       else:
         error += "\n•FILAS"
         if len(self.text_area_columnas.text) == 0:
           error += "\n•COLUMNAS"
    if len(error) > 0:
